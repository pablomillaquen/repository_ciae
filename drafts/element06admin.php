...
require_login();
require_once("../forms/elementdraft06.php");
...
$data->string_title = get_string('axis', 'local_repositoryciae');

$objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "axis" AND discussion_id = '.$discussionid);
...
if($objanswers){
    foreach($objanswers as $objanswer){
        ...
        $objanswer->answer = $optionsaxis[$objanswer->answer];
...
$mform = new elementdraft06_form();

if($fromform = $mform->get_data()){
    //Update answers to zero
    $objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "axis" AND discussion_id = '.$fromform->discussion_id);
    ...
    redirect("/local/repositoryciae/drafts/element06admin.php?discussionid=".$fromform->discussion_id, 'Cambios guardados', 10,  \core\output\notification::NOTIFY_SUCCESS);
}
