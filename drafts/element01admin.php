...
require_login();
require_once("../forms/elementdraft01.php");
...
$data->string_title = get_string('recomendedgrades', 'local_repositoryciae');

$objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "grades" AND discussion_id = '.$discussionid);

if($objanswers){
    foreach($objanswers as $objanswer){
        $user = $DB->get_record_sql('SELECT * FROM {user} WHERE id='.$objanswer->user_id);
        $objanswer->answer = $optionsgrades[$objanswer->answer];
...
$mform = new elementdraft01_form();

if($fromform = $mform->get_data()){
    //Update answers to zero
    $objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "grades" AND discussion_id = '.$fromform->discussion_id);
    ...
    $storedfile = $DB->insert_record('local_repositoryciae_answer', $newelement, true, false);
     ...
     if($objdraft){ //If draft exist, update
         $objdraft->grades = $fromform->answer;
    ...
    redirect("/local/repositoryciae/drafts/element01admin.php?discussionid=".$fromform->discussion_id, 'Cambios guardados', 10,  \core\output\notification::NOTIFY_SUCCESS);
}
