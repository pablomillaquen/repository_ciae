...
require_login();
require_once("../forms/elementdraft04.php");
...
$data->string_title = get_string('suggestions', 'local_repositoryciae');

$objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "suggestions" AND discussion_id = '.$discussionid);
...
$mform = new elementdraft04_form();

if($fromform = $mform->get_data()){
    //Update answers to zero
    $objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "suggestions" AND discussion_id = '.$fromform->discussion_id);
    ...
    $newelement->question = "suggestions";
    ...
    $storedfile = $DB->insert_record('local_repositoryciae_answer', $newelement, true, false);
     ...
     if($objdraft){ //If draft exist, update
         $objdraft->suggestions = $fromform->answer;
    ...
    redirect("/local/repositoryciae/drafts/element04admin.php?discussionid=".$fromform->discussion_id, 'Cambios guardados', 10,  \core\output\notification::NOTIFY_SUCCESS);
}
