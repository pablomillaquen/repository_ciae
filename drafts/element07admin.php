...
require_login();
require_once("../forms/elementdraft07.php");
...
$data->question = "learning";

$answers = array();
$objdiscussion = $DB->get_record_sql('SELECT * FROM {forum_discussions} WHERE id = '.$discussionid);
$data->objdiscussion_id = $discussionid;
$data->string_title = get_string('learning', 'local_repositoryciae');

$objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "learning" AND discussion_id = '.$discussionid);
...
if($objanswers){
    foreach($objanswers as $objanswer){
        ...
        $learn = null;
        $learning_ids = explode(",", $objanswer->answer);
        $i=0;
...
$mform = new elementdraft07_form();

if($fromform = $mform->get_data()){
    //Update answers to zero
    $objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "learning" AND discussion_id = '.$fromform->discussion_id);
    ...
    $newelement->question = "learning";
    $newelement->answer = implode(",",$fromform->answer);
    ...
    $storedfile = $DB->insert_record('local_repositoryciae_answer', $newelement, true, false);
     ...
     if($objdraft){ //If draft exist, update
         $objdraft->learning = implode(",",$fromform->answer);
    ...
    redirect("/local/repositoryciae/drafts/element07admin.php?discussionid=".$fromform->discussion_id, 'Cambios guardados', 10,  \core\output\notification::NOTIFY_SUCCESS);
}
