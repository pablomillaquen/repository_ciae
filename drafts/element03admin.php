...
require_login();
require_once("../forms/elementdraft03.php");
...
$data->question = "linguistic";

$answers = array();
$objdiscussion = $DB->get_record_sql('SELECT * FROM {forum_discussions} WHERE id = '.$discussionid);
$data->objdiscussion_id = $discussionid;
$data->string_title = get_string('linguistic', 'local_repositoryciae');

$objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "linguistic" AND discussion_id = '.$discussionid);
...
$mform = new elementdraft03_form();

if($fromform = $mform->get_data()){
    //Update answers to zero
    $objanswers = $DB->get_records_sql('SELECT * FROM {local_repositoryciae_answer} WHERE question = "linguistic" AND discussion_id = '.$fromform->discussion_id);
    ...
    $newelement->question = "linguistic";
    ...
    $storedfile = $DB->insert_record('local_repositoryciae_answer', $newelement, true, false);
     ...
     if($objdraft){ //If draft exist, update
         $objdraft->linguistic = $fromform->answer;
    ...
    redirect("/local/repositoryciae/drafts/element03admin.php?discussionid=".$fromform->discussion_id, 'Cambios guardados', 10,  \core\output\notification::NOTIFY_SUCCESS);
}
