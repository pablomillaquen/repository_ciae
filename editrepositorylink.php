...
$objmaterials = $DB->get_records('local_repositoryciae_links');
foreach($objmaterials as $mat){
    ...
    //Image
    if($mat->photo){
        $fileimage = $DB->get_record_sql("SELECT * FROM mdl_files WHERE itemid = ". $mat->photo ." AND filesize > 1 AND component = 'local_repositoryciae'  LIMIT 1");
        if($fileimage){
...
$PAGE->set_url('/local/repositoryciae/editrepositorylink.php');
...
echo $OUTPUT->render_from_template('local_repositoryciae/editrepositorylink', $data);
